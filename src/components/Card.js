...
const Card = ({children}) => (
  <div className="flex flex-col gap-4 min-w-0 break-words border-bzyellow border-l-stripe">
    {children}
  </div>
)

const CardImage = ({image, altText}) => {
  return <GatsbyImage image={getImage(image)} alt={altText} className="w-full"/>
}
